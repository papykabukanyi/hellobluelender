...
// Use command line arg for demo mode
const TEST_MODE = process.argv.includes('--demo');

// Set longer timeouts for axios
...
async function submitTestApplication(appData, index) {
  try {
    ...
    // Use the exact endpoint we know exists in the application    const submitUrl = `${API_URL}/application/submit`;
    console.log(`Submitting to ${submitUrl}`);
    
    // Set proper headers and timeout for the API request
    const config = {
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
      },
      timeout: 15000 // Increase timeout to 15 seconds for form submission
    };
    
    const response = await axios.post(submitUrl, appData, config);
...
async function checkServerRunning() {
  ...
  try {
    ...
    try {
      // Skip the health endpoint (it has errors)
      // Try the root endpoint directly - it's working based on the logs
      ...
      console.error('Error details when checking root endpoint:', err.message);
    }
