...
export async function POST(request: NextRequest) {
  try {
    ...
    const recipients = recipientsJson ? JSON.parse(recipientsJson) : [];    // Only use real recipients from the database, no defaults
    const emailRecipients = recipients.filter((r: any) => r.active).map((r: any) => r.email);
    
    // If there are recipients configured, send them the application
