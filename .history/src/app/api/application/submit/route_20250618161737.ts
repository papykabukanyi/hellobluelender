...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only use real recipients from the database, no defaults    // Filter active recipients and prioritize main admin recipient
    const allRecipients = recipients.filter((r: any) => r.active);
    
    // Find the main admin recipient (marked with isMainAdmin flag)
    const mainAdminRecipient = allRecipients.find((r: any) => r.isMainAdmin === true);
    
    // Prepare recipient list with main admin first
    let emailRecipients: string[] = [];
    
    // If main admin recipient is found, add it first
