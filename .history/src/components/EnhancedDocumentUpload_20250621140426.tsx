...
import DocumentScanner, { DocumentType, ExtractedDocumentData } from '@/lib/document-scanner/DocumentScanner';
    ...
    if (newFiles.length > 0 && scannerRef.current) {
      ...
      try {
        ...
        const result = await Promise.race([
          scannerRef.current.processDocument(newFiles[0]),
          new Promise<any>((resolve) => {
            // Fallback after 10 seconds if processing takes too long
            setTimeout(() => {
              console.warn('Document processing taking too long, using fallback');
              resolve({
