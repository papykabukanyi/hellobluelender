...
type FormDataType = z.infer<typeof businessLoanSchema> | z.infer<typeof equipmentLoanSchema>;
...
export default function LoanInfoForm({ onNext, onBack, formData = {}, loanType = 'Business' }: LoanInfoFormProps) {
  ...
  const {
    ...
    defaultValues: { ...formData, loanType } as FormDataType,
    mode: 'onChange'
  });
  ...
  useEffect(() => {
    setValue('loanType', loanType as LoanType);
  }, [loanType, setValue]);
  
  // Reset form with initial values when component mounts
  useEffect(() => {
    reset({ ...formData, loanType } as FormDataType);
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          {errors.loanType && (
            <p className="mt-1 text-sm text-red-500">{getErrorMessage('loanType')}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.loanAmount && (
            <p className="mt-1 text-sm text-red-500">{getErrorMessage('loanAmount')}</p>
          )}
        </div>
          ...
          <>
            <div>
              ...
              {errors.equipmentType && (
                <p className="mt-1 text-sm text-red-500">{getErrorMessage('equipmentType')}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.equipmentCost && (
                <p className="mt-1 text-sm text-red-500">{getErrorMessage('equipmentCost')}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.equipmentVendor && (
                <p className="mt-1 text-sm text-red-500">{getErrorMessage('equipmentVendor')}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.downPayment && (
                <p className="mt-1 text-sm text-red-500">{getErrorMessage('downPayment')}</p>
              )}
            </div>
          </>
        )}
          ...
          <>
            <div>
              ...
              {errors.monthlyRevenue && (
                <p className="mt-1 text-sm text-red-500">{getErrorMessage('monthlyRevenue')}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.timeInBusiness && (
                <p className="mt-1 text-sm text-red-500">{getErrorMessage('timeInBusiness')}</p>
              )}
            </div>
          </>
        )}
        ...
        <div className="md:col-span-2">
          ...
          <textarea
            id="loanPurpose"
            rows={3}
            className={`form-input ${errors.loanPurpose || (watchLoanType === 'Business' && errors.useOfFunds) ? 'border-red-500' : ''}`}
            {...register(watchLoanType === 'Business' ? 'useOfFunds' : 'loanPurpose')}
          ></textarea>
          {watchLoanType === 'Business' ? (
            errors.useOfFunds && (
              <p className="mt-1 text-sm text-red-500">{getErrorMessage('useOfFunds')}</p>
            )
          ) : (
            errors.loanPurpose && (
              <p className="mt-1 text-sm text-red-500">{getErrorMessage('loanPurpose')}</p>
            )
          )}
        </div>
      </div>
