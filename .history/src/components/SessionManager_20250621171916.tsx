...
export default function SessionManager({ user }: SessionManagerProps) {
  ...
  useEffect(() => {
    ...
    const handleActivity = () => {
      ...
      // If warning is showing but user became active, reset the warning
      if (showWarning) {
        setShowWarning(false);
        setTimeLeft(60); // Reset to 1 minute countdown
    ...
    // Save session activity to localStorage as a backup
    const persistActivity = () => {
      localStorage.setItem('lastActivity', lastActivityRef.current.toString());
    };
    
    // Set interval to persist activity (every 2 minutes)
    const persistInterval = setInterval(persistActivity, 2 * 60 * 1000);
    
    // Try to restore activity from localStorage on component mount
    ...
    const checkSessionTimeout = () => {
      ...
      // If inactive for 89 minutes (1 minute before 90 minutes), show warning
      if (inactiveTime > 89 * 60 * 1000 && !showWarning) {
        setShowWarning(true);
        ...
        // Set timeout to logout after 1 more minute of inactivity
