...
export class DocumentScanner {
  ...
  private async _processDocumentInternal(file: File): Promise<ExtractedDocumentData> {
    try {
      ...
      const classification = this.classifyDocumentType(text);
      if (!classification || typeof classification !== 'object') {
        throw new Error('Document classification failed');
      }
      
      const type = classification.type || DocumentType.UNKNOWN;
      ...
      console.error('Error in _processDocumentInternal:', error);
