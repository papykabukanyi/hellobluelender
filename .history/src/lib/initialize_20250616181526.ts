...
export async function initializeAdminData() {
  try {
    ...
    const smtpConfigExists = await redis.exists('smtp:config');
    ...
    if (!smtpConfigExists) {
      ...
      const defaultSMTPConfig: SMTPConfig = {
        ...
        username: process.env.SMTP_USER || 'user@example.com',
        password: process.env.SMTP_PASS || 'password',
        fromEmail: process.env.SMTP_FROM_EMAIL || 'noreply@bluelender.com',
        fromName: process.env.SMTP_FROM_NAME || 'Blue Lender',
        secure: process.env.SMTP_SECURE === 'true',
      };
      
      // Store in Redis
      await redis.set('smtp:config', JSON.stringify(defaultSMTPConfig));
