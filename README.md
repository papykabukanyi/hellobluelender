...
2. In a separate terminal, run the automated test:
   ...
   Alternatively, you can run these specific test scripts:

   ```bash
   npm run test:demo              # Run test in demo mode (no server required)
...
- If you just want to see how the test would work without starting the server, use `npm run test:demo`
