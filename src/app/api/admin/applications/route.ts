  ...
  try {
    ...
    if (
      ...
      application.personalInfo.email
        ...
        const statusEmailResult: {success: boolean, messageId?: string, error?: any} = await sendEmail({
          to: applicantEmail,
        ...
        if (!statusEmailResult.success) {
          console.error('Failed to send status update email:', statusEmailResult.error);
          ...
          console.log(`Status update email sent to applicant (${applicantEmail}): Application ${status}, Message ID: ${statusEmailResult.messageId}`);
        }
      } catch (emailError) {
