...
export default function EnhancedDocumentUpload({
  ...
  const handleFiles = useCallback(async (newFiles: File[]) => {
    ...
    if (newFiles.length > 0) {
      ...
      // Create scanner if not yet initialized
      if (!scannerRef.current) {
        try {
          scannerRef.current = new DocumentScanner();
        } catch (err) {
          console.error("Failed to initialize document scanner:", err);
        }
      }
      ...
      try {
        ...
        // Try to process document if scanner is available
        if (scannerRef.current) {
          try {
            result = await scannerRef.current.processDocument(newFiles[0]);
          } catch (processingError) {
            console.error("Document analysis failed:", processingError);
            // We'll use the fallback result defined above
          }
        }
        ...
        // Always proceed with save regardless of analysis results
        if (onSave) {
          onSave({
        ...
        setErrorMessage('Document uploaded, but analysis encountered an issue. Your application can still proceed.');
  ...
  }, [files, maxFiles, acceptedFileTypes, maxFileSize, onDataExtracted, onSave]);
